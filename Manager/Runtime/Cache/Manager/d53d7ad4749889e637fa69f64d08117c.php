...
<div class="container-fluid">
	...
	<div class="row-fluid tool">
		<a href="/Manager/System/NodeAdd" class="btn btn-info" aria-label="Left Align"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 新增模块</a>		
	</div>
	<!-- 页面主内容 -->
	<div class="row-fluid">
		<div class="table-responsive">
			<table class="table table-bordered table-hover">
				<tr>
					<th></th><th>模块名</th><th>标题</th><th>状态</th><th>类别</th><th>排序ID</th><th>备注</th><th>操作</th>
				</tr>
				<?php if(is_array($list)): foreach($list as $key=>$item): ?><tr>
						<td><?php echo ($key+1); ?></td>
						<td><?php echo ($item["name"]); ?></td>
						<td><?php echo ($item["title"]); ?></td>
						<td><?php echo ($item['status']==1?'启用':'禁用'); ?></td>
						
						<td>
							<?php switch($item["level"]): case "1": ?>项目<?php break;?>
								<?php case "2": ?>模块<?php break;?>
								<?php case "3": ?>操作<?php break;?>
								<?php default: ?>--<?php endswitch;?>
						</td>
						<td><?php echo ($item["sort"]); ?></td>
						<td><?php echo ($item["remark"]); ?></td>
						<td>
							<a onclick="Del(<?php echo ($item["id"]); ?>)">删除</a>&nbsp;<a href="/Manager/System/NodeEdit/id/<?php echo ($item["id"]); ?>">修改</a>
						</td>
